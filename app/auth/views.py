...
def profile():
    ...
    return render_template('auth/profile.html', title="Профіль", username=username, cookies=cookies)
...
@auth_bp.route('/add-cookie', methods=['POST'])
def add_cookie():
    if 'username' not in session: 
        return redirect(url_for('auth.login'))
...
@auth_bp.route('/delete-cookie', methods=['POST'])
def delete_cookie():
    if 'username' not in session:
        return redirect(url_for('auth.login'))
